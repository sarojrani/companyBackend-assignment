...
const userContact = async (req, res) => {
  ...
  let phoneRegex = /^(\+91[\-\s]?)?[0]?(91)?[6789]\d{9}$/.test(mobileNo);
...
const listContactUser = async (req, res) => {
  ...
  if (!data) {
    return res
      .status(400)
      .send({ status: false, msg: "list is empty or first add contact" });
  }
...
const filterContact = async (req, res) => {
  ...
  const MatchData = await userModel.aggregate([
    { $match: { $or: [{ name: name }, { mobileNo: mobileNo }] } },
  ]);
  if (!MatchData) {
...
const updateContact = async (req, res) => {
  ...
  // }
  const saveData = await userModel.findOneAndUpdate(
    { _id: id },
    { $set: { name, mobileNo } },
    { new: true }
  );
